...
class MyLounges extends StatefulWidget {
  ...
  const MyLounges({required this.id, required this.type, Key? key})
      : super(key: key);
              ...
              if (snapshot.data == null) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int i) {
                    final date = DateTime.parse(snapshot.data[i].fechaCreacion);
                      ...
                      Padding(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(snapshot.data[i].descripcion,
                                style: TextStyle(
                                    fontSize: 15.0, color: _colorPalette.text),
                                textAlign: TextAlign.justify),
                            isThreeLine: true,
                            trailing: Text(formatterFront.format(date),
                                style: TextStyle(
                                    fontSize: 15.0, color: _colorPalette.text),
                                textAlign: TextAlign.justify),
                            onTap: () {
                                ...
                                MaterialPageRoute(
                                    builder: (BuildContext context) =>
                                        LoungeDetail(
                                          id: widget.id,
                                          type: widget.type,
                                          salaId: snapshot.data[i].id,
                                          salaName: snapshot.data[i].titulo,
                                        )),
                              );
                            },
                          )),
